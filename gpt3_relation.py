...
def auto_generate_example(
    ...
    for tmp_dict in example_list:
        ...
        if not reasoning:
            ...
            tmp_query = (
                'What are the clues that lead the relation between "'
                + entity1
                + '" and "'
                + entity2
                + '" to be '
...
def generate_ft_example(tmp_dict, ft_dict, reltoid, idtoprompt, demo, args):
    ...
    for tmp_dict in example_list:
        ...
        if not args.reasoning or label_other == 1:
            ...
            prompt_query = tmp_example.get_self_error(
                tmp_dict, demo, reltoid, idtoprompt, args
            )
        else:
            # tmp_query = "\nGiven the sentence: \"" + string + "\", What are the clues that lead the relation between \"" + entity1 + "\" and \"" + entity2 + "\" to be " + idtoprompt[reltoid[rel]] + "?"
            tmp_query = (
...
def generate_lm_example(
    ...
    example_list = find_lmknn_example(
        gpu_index_flat, tmp_dict, train_dict, train_sentences, k
    )
...
def generate_knn_example(
    ...
    # assert False
    # print(len(train_list))
    example_list = find_knn_example(
        knn_model, tmp_dict, train_dict, k, args.entity_info
    )
...
def get_binary_select(pred, tmp_dict, demo, knn_list, reltoid, idtoprompt, args):
    ...
    prompt_list += (
        test_example.discriminator + idtoprompt[pred] + "?" + test_example.answer
    )
...
def run(reltoid, idtoprompt, store_path, args):
    ...
    if args.use_ft:
        ...
        if args.no_na:
            if args.task == "semeval":
                train_list = [
                    x for x in train_list if reltoid[x["relations"][0][0][4]] != 0
                ]
            else:
    ...
    for run in range(args.num_run):
        if args.fixed_example:
            example_prompt = auto_generate_example(
                example_dict,
                reltoid,
                idtoprompt,
                args.num_per_rel,
        ...
        for tmp_dict in test_examples:
            ...
            if (
                tmp_dict["relations"] != [[]]
                and tmp_dict["relations"][0][0][4] == "Other"
                and args.no_na
            ):
                num += 1
                continue
            ...
            if args.use_knn:
                if args.use_ft:
                    example_prompt, tmp_knn, label_other, knn_list = (
                        generate_ft_example(
                            tmp_dict, ft_dict, reltoid, idtoprompt, demo, args
                        )
                    )
                elif args.lm_mask:
                    example_prompt, tmp_knn, label_other, knn_list = (
            ...
            prompt_list, subject, target = generate_select_auto_prompt(
                tmp_dict, example_prompt, reltoid, args.no_na, args.reasoning, args
            )
            # results, probs = demo.get_multiple_sample(prompt_list)
            ...
            if args.var and label_other == 1:
                ...
                pred, prob_on_rel, prob, error = get_results_select(
                    demo, prompt_list, reltoid, idtoprompt, args.verbalize, args
                )
                if error:
                    azure_error.append(tmp_dict["doc_key"])
                if args.discriminator and pred != 0:
                    ori_pred = pred
                    pred, prob = get_binary_select(
                        pred, tmp_dict, demo, knn_list, reltoid, idtoprompt, args
                    )
                    if pred != ori_pred:
                        print("work!")
            ...
            if preds[-1] != labels[-1]:
                if args.store_error_reason:
                    error_reason = instance(tmp_dict).get_error_reason(
                        preds[-1],
                        tmp_dict,
                        example_prompt,
                        demo,
                ...
                if args.store_error_reason:
                    correct_reason = instance(tmp_dict).get_correct_reason(
                        demo, idtoprompt, reltoid, args
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        required=True,
        choices=["ace05", "semeval", "tacred", "scierc", "wiki80"],
    )
    ...
    print("done parsing, start script")
    ...
    if args.no_na and args.num_na != 0:
        ...
        print(args.num_na)
        assert False
    store_path = "./results/knn_{}_results/test={}_knn={}_reverse={}_nona={}_var={}_{}_{}_seed={}_{}_randomlabel={}_fixedex={}_fixedtest={}_Reason={}_Verbalize={}_Entityinfo={}_structure={}_useft={}_selferror={}_usedev={}_discri={}_{}".format(
        args.task,
        args.num_test,
        args.k,
        args.reverse,
        ...
        args.seed,
        args.model,
        str(args.random_label),
        str(args.fixed_example),
        str(args.fixed_test),
        str(args.reasoning),
        args.verbalize,
        args.entity_info,
    ...
    short_store_path = "./results/knn_{}_results/config1".format(args.task)
    print(os.listdir("./results"))
    if not os.path.exists(short_store_path):
        os.mkdir(short_store_path)
    ...
    if args.task == "semeval":
        ...
        run(semeval_reltoid, semeval_idtoprompt, short_store_path, args)
    elif args.task == "ace05":
        # example_dataset = "./dataset/ace05/test.json"
        # dataset = "./dataset/ace05/ace05_0.2/ace05_0.2_test.txt"
        run(ace05_reltoid, ace05_idtoprompt, short_store_path, args)
    elif args.task == "tacred":
        run(tacred_reltoid, tacred_idtoprompt, short_store_path, args)
    elif args.task == "scierc":
        run(scierc_reltoid, scierc_idtoprompt, short_store_path, args)
    elif args.task == "wiki80":

        run(wiki_reltoid, wiki_idtoprompt, short_store_path, args)
